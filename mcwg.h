...
protected:
  ...
  uint32_t pull_int(const std::vector<uint8_t> &vec, uint32_t pos) {
    assert(pos <= vec.size() - sizeof(uint32_t) &&
           "position must be less than vec size");
    uint32_t ret = 0;
    ret |= vec[pos++];
    ret |= vec[pos++] << 8;
    ret |= vec[pos++] << 16;
  ...
  std::string pull_string(const std::vector<uint8_t> &vec, const int pos,
                          const int len) {
    auto ret = std::string();
...
public:
  uint32_t &operator[](const char key) { return _map[key]; }
  AlphabetMap() {
...
public:
  Generator(const int order, const std::vector<Model> &models)
      : _models(models), _order(order) {}
